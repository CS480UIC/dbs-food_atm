...
public class warehouseServletDelete extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public warehouseServletDelete() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(method.equals("search"))
		{
			...
			if(warehouse.getwarehouse_id()!=null){
						...
						request.getRequestDispatcher("/jsps/warehouse/warehouse_delete_output.jsp").forward(request, response);			
				}
				else{
		...
		{	
			try {
				warehouseDao.delete(request.getParameter("warehouse_id"));
			} catch (ClassNotFoundException e1) {
				e1.printStackTrace();
			} catch (InstantiationException e1) {
				e1.printStackTrace();
